...
export const maxDuration = 600; // 允许最长 10 分钟（支持长录音上传+转写）
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await transcribeWithTencentFlash(wavBuffer, sampleRate);
    console.log('[Tencent] 完成，说话人数:', result.utterances.map(u => u.speaker_id).filter((v, i, a) => a.indexOf(v) === i).length);

    return NextResponse.json({ success: true, ...result });
  } catch (error: any) {
    console.error('[Tencent] Error:', error.message, '| cause:', error.cause?.message ?? error.cause);
    return NextResponse.json(
